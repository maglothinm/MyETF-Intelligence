...
## 2026-09-09 — Bind credential-free Legislative recovery to immutable evidence

**Decision:** Preserve failed run `065d5330-abca-4eda-b683-64e85f2dcbe7` and its original side-effect flag. Permit one explicit recovery only through a code-pinned receipt binding its entire run record, generation 232 parent and immutable execution/image evidence that notification credentials were absent. Check the full post-parent run inventory under the existing writer lock; any later run or moved parent invalidates the case. Record adjudication in the new run and successful snapshot, never by modifying the old failure.

**Reason:** The Senate source is accessible again from the production network, but the old runner conservatively marked even credential-free tracker invocations as potentially delivering alerts. Source counts alone do not resolve delivery uncertainty; the immutable execution, image configuration and notification code do resolve it for this exact incident.

**Consequence:** Future tracker failures use actual notification capability to select the side-effect flag, retaining the guard whenever both delivery credentials exist and notifications are enabled. Complete-source validation, accepted state, single-writer ownership, scheduled settings and AI delivery semantics are preserved. Implementation does not itself establish production recovery; the controlled successor and dashboard publication require separate verification. See [the recovery record](incidents/2026-09-09-legislative-recovery.md).
