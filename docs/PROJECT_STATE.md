...
Source fixes are implemented on `codex/oge-pdf-repair-20260919`, based on `8ca6c4b`. Official OGE PDF links now receive direct access metadata while the old URL-slot calculation preserves every listing ID. Existing producer passes append metadata corrections to retained PDFs, retry the affected old OCR failures once, and prioritize accessible documents over gated requests. Readable empty-password PDFs pass inspection; password-required/malformed files and all existing resource bounds remain enforced. Successful OCR caches and pending owner confirmations retain their version.

Local verification: **298 Python tests passed, 21 environment-dependent skips; 4 Node tests passed**. Actual OCR completed 2/2 pages of the previously rejected House sample and 3/3 pages of an OGE sample. A copy of the public ledger showed exactly 340 metadata corrections while preserving all 5,144 IDs and unrelated fields, with no writes on a repeat pass. Canonical CI/merge is pending at this checkpoint.
...
Next: verify canonical CI/merge, then perform a fresh coordinated release and independent live acceptance once deployment access is available. Preserve IDs, historical alerts, snapshots, personal review history and all closed journals. OGE layout acceptance and owner correction/import/independent cleanup verification remain open. [Repair details and evidence](releases/2026-09-19-oge-pdf-repair.md).
