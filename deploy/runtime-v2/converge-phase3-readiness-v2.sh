...
required=(
  ...
  GITHUB_SHA GITHUB_RUN_ID GITHUB_WORKSPACE
)
for name in "${required[@]}"; do
  [[ -n "${!name:-}" ]] || { echo "Missing ${name}." >&2; exit 1; }
done

work="${RUNNER_TEMP:-/tmp}/phase3-readiness-v2-${GITHUB_RUN_ID}"
rm -rf "${work}"
mkdir -p "${work}"
cd "${work}"
...
cleanup() {
  ...
  if [[ "${temporary_access}" == "true" ]]; then
    gcloud logging views remove-iam-policy-binding "${LOG_VIEW}" \
      ...
      --role roles/logging.viewAccessor --quiet >/dev/null 2>&1
...
assert_admin_baseline() {
  local c
  c="$(container_from_file "$1")"
  [[ "$(jq -c '.command // []' <<<"${c}")" == '["python"]' ]] || {
    echo "Unexpected admin command baseline." >&2
    exit 1
  }
  [[ "$(jq -c '.args // []' <<<"${c}")" == '["-m","runtime_v2","init-db","--with-vault"]' ]] || {
...
admin_before_container="$(container_from_file admin-before.json)"
runtime_image="$(jq -r '.image // empty' <<<"${admin_before_container}")"
[[ -n "${runtime_image}" ]] || { echo "Admin image is absent." >&2; exit 1; }

# Terraform already declares PRIVATE_IP=true. Reconcile only this observed deployment drift.
...
for job in "${runtime_jobs[@]}"; do
  ...
  gcloud run jobs describe "${job}" \
    --project "${PROJECT_ID}" --region "${REGION}" \
    --format=json > "${before}"
  c="$(container_from_file "${before}")"
  current="$(private_ip_from_container <<<"${c}")"
  if [[ "${current,,}" != "true" ]]; then
    gcloud run jobs update "${job}" \
      --project "${PROJECT_ID}" --region "${REGION}" \
      --update-env-vars=PRIVATE_IP=true --quiet >/dev/null
...
for job in "${runtime_jobs[@]}"; do
  ...
  gcloud run jobs describe "${job}" \
    --project "${PROJECT_ID}" --region "${REGION}" \
    --format=json > "${after}"
  c="$(container_from_file "${after}")"
  [[ "$(private_ip_from_container <<<"${c}" | tr '[:upper:]' '[:lower:]')" == "true" ]] || {
...
# Establish bounded access for exactly one read-only status execution.
cleanup
...
for attempt in $(seq 1 30); do
  gcloud logging read \
    ...
    --project "${PROJECT_ID}" --order=asc --limit=500 \
...
# Remove temporary authority before interpreting acceptance evidence.
cleanup
temporary_access=false
verify_temporary_access_removed
assert_schedulers_paused
...
RUNTIME_IMAGE="${runtime_image}" python - <<'PY'
import hashlib
import json
import os
...
remote_digest="$(gcloud storage cat "${receipt_uri}" | sha256sum | awk '{print $1}')"
