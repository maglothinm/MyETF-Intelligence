...
WORKFLOW = Path('.github/workflows/phase3_build_current_runtime_image_v2.yml')
...
def test_build_v2_is_canonical_self_path_scoped_main_push() -> None:
    text = _text()
    assert 'name: Phase 3 build current Runtime image v2' in text
    assert 'branches:' in text and '- main' in text
    assert '".github/workflows/phase3_build_current_runtime_image_v2.yml"' in text
...
def test_build_v2_keeps_submitter_and_executor_separate() -> None:
    text = _text()
...
def test_build_v2_fetches_exact_commit_inside_no_source_build() -> None:
    text = _text()
...
def test_build_v2_requires_private_routing_source_and_immutable_digest() -> None:
    text = _text()
