...
(() => {
  ...
  function cell(row,field,type){let v=get(row,field);if(["investor_edge_relevant_followable_alpha","investor_edge_sector_alpha","investor_edge_score"].includes(field)&&(["insufficient_data","unavailable","disabled","error","neutral"].includes(row.investor_edge_status)||row.investor_edge?.minimum_sample_met===false))v=null;if(field==="final_score")v=v??row.score;
    ...
    if(type==="discovery"){
      const d=row.information_value_at_discovery||{},p=d.transaction_to_discovery_percent;
      const summary=d.status==="measured"&&typeof p==="number"?p.toFixed(2)+"% since transaction reference":"Unknown";
      return `<strong>${esc(summary)}</strong><small>${esc(title(d.reason||"missing evidence"))}</small><details><summary>Discovery evidence</summary><pre>${esc(JSON.stringify(d,null,2))}</pre></details>`;
    }
