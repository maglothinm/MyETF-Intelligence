...
window.PT = (() => {
  ...
  // Use the retained execution time, never the localized display label. The
  // model normally folds started_utc into finished_utc when finish is missing.
  const runTime = run => [run.finished_utc,run.started_utc].find(value=>typeof value==="string" && Number.isFinite(Date.parse(value)));
