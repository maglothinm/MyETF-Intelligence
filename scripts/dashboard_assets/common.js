...
window.PT = (() => {
  ...
  const analysisHref = (row,prefix="index.html") => {
    const id=typeof row?.analysis_id==="string"?row.analysis_id:"";
    if(!id.trim()||id.length>500)return "";
    try{return `${prefix}#signals?analysis=${encodeURIComponent(id)}`;}catch{return "";}
  };
  ...
  function signalCard(row,compact=false) {
    ...
    const receipt=compact?"":analysisHref(row,"");
    let graphic="";
  ...
  return {el,esc,HELP,helpAttrs,helpButton,numeric,number,money,percent,title,date,age,durationMinutes,branchLabel,triggerLabel,healthAt,createHealthClock,healthViewKey,cadenceText,healthDetail,monitoringSummary,safeUrl,link,analysisHref,filingActions,workflowUrl,checkedJson,statusText,fact,emptySignals,signalCard,healthCards,replay,brief,validateModel,isCoarsePointer,setupDialogsAndTooltips};
})();
